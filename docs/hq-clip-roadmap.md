...
| 🔄 | Security: enforce service-to-service auth (signed JWT or API key). | - FastAPI now honors `CLIP_SERVICE_AUTH_TOKEN` / `CLIP_SERVICE_TOKEN` and rejects missing or invalid bearer headers.<br>- Follow-up: add Pytest coverage to simulate 401 vs 200 once the backend test harness is in place. |
